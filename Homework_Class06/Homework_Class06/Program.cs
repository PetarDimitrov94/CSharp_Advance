...
class Program
{
    static void Main()
    {
        ...
        List<int> uniqueCylinderValue = cars
            .Select(c => c.Cylinders)
            .Distinct()
            .ToList();

        foreach (int cyl in uniqueCylinderValue)
        {
            Console.WriteLine($"Cylinders: {cyl}");
        }
        ...
        foreach(string model in carNames)
        {
            Console.WriteLine($"Car Names: {model}");
        }

        Console.WriteLine("======================================================================");
        //4.Check if there are any cars with horsepower greater than 300.
        bool horsepower = cars.Any(c => c.HorsePower > 300);

        Console.WriteLine("======================================================================");
        //5.Find the car with the highest horsepower.
        List<Car> mostPowerfulCarList = cars
            .OrderByDescending(c => c.HorsePower)
            .Take(1)
            .ToList();

        foreach (Car car in mostPowerfulCarList)
        {
            Console.WriteLine($"Most powerful car: {car.Model} with {car.HorsePower} HP");
        }
        Console.WriteLine("======================================================================");
        //6.Filter all "Chevrolet" cars and order them by weight in descending order.
        List<Car> chevroletCars = cars
            .Where(c => c.Model.StartsWith("Chevrolet"))
            .OrderByDescending (c => c.Weight)
            .ToList();

        foreach (Car car in chevroletCars)
        {
            Console.WriteLine($"{car.Model} - Weight: {car.Weight}");
        }
        Console.WriteLine("======================================================================");
        //7.Find the car with the longest model name.
        List<Car> longestModelName = cars
            .OrderByDescending(c => c.Model.Length)
            .Take(1)
            .ToList();
        foreach (Car car in longestModelName)
        {
            Console.WriteLine($"Car with longest model name : {car.Model}");
        }
        Console.WriteLine("======================================================================");

        //8.Group cars by their origin and then order the groups by the number of cars in each group, in ascending order.
        //Za ova iskreno ne znam dali treba vaka
        List<Car> groupedCars = cars
            .GroupBy(c => c.Origin)
            .OrderBy(group => group.Count())
            .SelectMany(group => group)
            .ToList();

        foreach (Car car in groupedCars)
        {
        ...
        List<Car> topFiveCars = cars
            .OrderByDescending(c => c.HorsePower)
            .Take(5)
            .ToList();

        foreach (Car car in topFiveCars)
        {
        ...
        Car carWithHighestAcceleration = cars
            .OrderByDescending(c => c.AccelerationTime)
            .First();


        Console.WriteLine($"{carWithHighestAcceleration.Model} - {carWithHighestAcceleration.AccelerationTime} sec");
        Console.WriteLine("======================================================================");
        //11.Select only the model and horsepower of cars with horsepower greater than 200.
        List<ModelAndHorsepowerOfCar> carsHorsepowerGreaterThan200 = cars
            .Where(c => c.HorsePower > 200)
            .Select( c => new ModelAndHorsepowerOfCar { Model = c.Model, HorsePower = c.HorsePower})
            .ToList();

        foreach ( ModelAndHorsepowerOfCar item in carsHorsepowerGreaterThan200)
        {
        ...
        List<string> uniqueOrigins = cars
            .Select(c => c.Origin)
            .Distinct()
        ...
        List<Car> carsWithMore4Cylinders = cars
            .Where(c => c.Cylinders > 4)
            .OrderBy(c => c.Origin)
            .ThenBy(c => c.HorsePower)
            .ToList();

        foreach (Car car in carsWithMore4Cylinders)
        {
            Console.WriteLine($"{car.Model} - {car.Cylinders} cylinders - {car.Origin} - {car.HorsePower} HP");
        }

        Console.WriteLine("======================================================================");

        //14.Filter all cars that have more than 6 Cylinders not including 6 after that Filter all cars that have exactly 4 Cylinders and have HorsePower more then 110.0.
        //Join them in one result.
        List<Car> sixCylindersCars = cars
             .Where(c => c.Cylinders == 6)
             .ToList();

        List<Car> fourCylindersHighHpCars = cars
        ...
        foreach (Car car in combinedResult)
        {
            Console.WriteLine($"{car.Model} - Cylinders: {car.Cylinders}, HorsePower: {car.HorsePower}");
        }
        ...
        List<Car> theMostPowerfulCars = cars
            .Where(c => c.HorsePower > 200 && c.MilesPerGalon > 0)
            .ToList();

        double minMilesPerGalon = theMostPowerfulCars.Min(c => c.MilesPerGalon);
        double maxMilesPerGalon= theMostPowerfulCars.Max(c => c.MilesPerGalon);
        double avgMilesPerGalon = theMostPowerfulCars.Average(c => c.MilesPerGalon);

        Console.WriteLine("Cars with more than 200 HP:");
        Console.WriteLine($"Min MPG: {minMilesPerGalon}");
        Console.WriteLine($"Max MPG: {maxMilesPerGalon}");
        Console.WriteLine($"Average MPG: {avgMilesPerGalon}");

        Console.WriteLine("======================================================================");

        //16.Be creative and write requirement of your own choice :) (only one catch, must use at least 3 LINQ methods)

        //Find all European cars with Weight less than 2200, order them by AccelerationTime ascending, and select their Model and Weight.
        var lightEuropeanCars = cars
            .Where(c => c.Origin == "Europe" && c.Weight < 2200)
            .OrderBy(c => c.AccelerationTime)
            .Select(c => new { c.Model, c.Weight })
            .ToList();

        foreach (var car in lightEuropeanCars)
        {
            Console.WriteLine($"Model: {car.Model}, Weight: {car.Weight}");
        }
        Console.WriteLine("======================================================================");

        //17.Be creative and write requirement of your own choice :) (only one catch, must use at least 3 LINQ methods)

        // Find all Japanese cars with model names longer than 10 characters, Selects their model and horsepower, Orders them by horsepower descending
        // And stores the result in a list.

        var result = cars
        .Where(c => c.Origin == "Japan" && c.Model.Length > 10)           
        .OrderByDescending(c => c.HorsePower)                             
        ...
        {
            Console.WriteLine($"Model: {car.Model}, HP: {car.HorsePower}");
        }
